...
resource "vsphere_virtual_machine" "controller" {
  ...
  name             = "${split(".ova", basename(var.aviOvaFile))[0]}-${count.index}"
  ...
  guest_id = "guestid-${split(".ova", basename(var.aviOvaFile))[0]}-${count.index}"


  disk {
    size             = var.controller["disk"]
    label            = "controller-${split(".ova", basename(var.aviOvaFile))[0]}-${count.index}.lab_vmdk"
